...
import { adminDelteRequest } from '../utils/adminForm';

const Admin = () => {
  const adminAccounts = useSelector(state => state.userData.adminAccounts);
  const loader = useSelector(state => state.userData.loader);
  const dispatch = useDispatch();

  const deleteAdmin = async (id) => {
    await adminDelteRequest(dispatch, id);
  }
                ...
                return (
                  <tr key={index} className="border-t border-gray-500/20 hover:bg-gray-100">
                    <td className="md:px-4 pl-2 md:pl-4 py-3 truncate">
                      <span className="truncate w-full capitalize">{user.name}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      {
                        !user.name.toLowerCase().includes('sachin') &&
                        <label onClick={() => deleteAdmin(user._id)} className="relative inline-flex items-center cursor-pointer text-gray-900">
